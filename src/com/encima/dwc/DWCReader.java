...
import com.encima.dwc.HibernateHelper;
...
//				Matching all keys, could be stored in a properties file, with the data type to cast to
				if(entry.getKey().equalsIgnoreCase("eventID")) {
					...
					Date date = new SimpleDateFormat("yyyy-MM-dd").parse(entry.getValue());
					occ.setEventDate(date);
				}else if(entry.getKey().equalsIgnoreCase("eventTime")) {
					Date date = new SimpleDateFormat("HH:mm:ss").parse(entry.getValue());
					occ.setEventTime(date);
				...
				if(entry.getKey().equalsIgnoreCase("identifiedBy")) {
					id.setIdentifiedBy(Integer.parseInt(entry.getValue()));
					...
					id.setSpeciesID(Integer.parseInt(entry.getValue()));
				}
			}
			occs.add(occ);
			hh.add(id);
...
//	  String path = "/Users/encima/Dropbox/images/dwc_stuff/dwc_arch";
